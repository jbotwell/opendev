...
mod tests {
    ...
    #[tokio::test]
    async fn test_insert_before_symbol_tool() {
        ...
        let tool = InsertBeforeSymbolTool;
        let ctx = ToolContext::new(tmp.path());
        let args = make_args(&[
            ("file_path", serde_json::json!(file_path.to_str().unwrap())),
            ("symbol_name", serde_json::json!("second")),
            (
                "content",
                serde_json::json!("/// Documentation for second\n"),
            ),
        ]);
        ...
        assert!(content.contains("/// Documentation for second\nfn second()"));
    }

    #[tokio::test]
    async fn test_insert_after_symbol_tool() {
        let tmp = TempDir::new().unwrap();
        let file_path = tmp.path().join("test.rs");
        std::fs::write(
            &file_path,
        ...
        let tool = InsertAfterSymbolTool;
        let ctx = ToolContext::new(tmp.path());
        let args = make_args(&[
            ("file_path", serde_json::json!(file_path.to_str().unwrap())),
            ("symbol_name", serde_json::json!("first")),
        ...
        let content = std::fs::read_to_string(&file_path).unwrap();
        assert!(content.contains("fn inserted()"));
        // inserted should appear between first and second
        let first_pos = content.find("fn first").unwrap();
        let inserted_pos = content.find("fn inserted").unwrap();
        let second_pos = content.find("fn second").unwrap();
        assert!(first_pos < inserted_pos);
        assert!(inserted_pos < second_pos);
    }

    #[tokio::test]
    async fn test_insert_missing_file_path() {
        let tool = InsertBeforeSymbolTool;
        let ctx = ToolContext::new("/tmp");
        let args = make_args(&[
            ("symbol_name", serde_json::json!("foo")),
            ("content", serde_json::json!("bar")),
        ]);
    ...
    #[tokio::test]
    async fn test_insert_symbol_not_found() {
        let tmp = TempDir::new().unwrap();
        let file_path = tmp.path().join("test.rs");
        ...
        let args = make_args(&[
            ("file_path", serde_json::json!(file_path.to_str().unwrap())),
            ("symbol_name", serde_json::json!("nonexistent")),
            ("content", serde_json::json!("// comment")),
        ]);
